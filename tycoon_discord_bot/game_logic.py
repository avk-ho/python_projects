...
class Gamelogic:
    ...
    def start_turn(self, round):
        ...
        while not end_turn:
            ...
            and turn.last_play == self.current_turn_player:
                end_turn = True
                continue
            ...
            if current_play.nb == 4:
                ...
                if self.revolution:
                    print(
                        "Revolution ! All cards' strength is reversed (excluding Jokers).")
                else:
            ...
            # joker 3S counter
            if turn.last_play is not None \
            and (turn.last_play.cards[0] == "JK1" or turn.last_play.cards[0]) == "JK2" \
    ...
    def play_cards(self, turn):
        # TO DELETE
        self.current_turn_player.display_hand(hidden=False)
        ...
        while not valid_play:
            ...
            for card in input_cards:
                ...
                if play_value == 0:
                    ...
                    print(
                        f"Incompatible cards played. {play_value} card with a {DECK[card]['value']} card.")
                    valid_values = False
                    break
    ...
    ### display hands of players, hidden or not (bool)
    def display_players_hands(self, hidden=True):
        for player in self.players:
            player.display_hidden_hand(hidden)


    # TO DO: discord implementation
    ### depending on players' status at start of round 2 and 3, exchange cards
    def redistribute_cards(self):
        tycoon = self.last_players_status["Tycoon"]
        rich = self.last_players_status["Rich"]
        poor = self.last_players_status["Poor"]
        beggar = self.last_players_status["Beggar"]
        ...
        while not valid_trade_tycoon:
            tycoon_trade_input = input(
                f"Please choose 2 cards to give (example: 3H 5D):\n")
